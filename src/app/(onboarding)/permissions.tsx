...
import { Linking, PermissionsAndroid, Platform, StyleSheet, Switch, TouchableOpacity, View } from 'react-native';
...
const PERMISSIONS: PermissionDef[] = [
  {
    ...
    androidPermissions: API_LEVEL >= 31
  ...
  },
  {
    ...
    androidPermissions: API_LEVEL >= 33
...
async function requestOne(def: PermissionDef): Promise<boolean> {
  ...
  if (def.androidPermissions.length === 1) {
    const result = await PermissionsAndroid.request(def.androidPermissions[0] as any);
    if (result === PermissionsAndroid.RESULTS.NEVER_ASK_AGAIN) {
      Linking.openSettings();
    }
    return result === PermissionsAndroid.RESULTS.GRANTED;
  }
  const results = await PermissionsAndroid.requestMultiple(def.androidPermissions as any);
    ...
    <ThemedView style={styles.screen}>
      <View style={styles.content}>
        ...
        <View style={styles.list}>
          {PERMISSIONS.filter((def) => def.androidPermissions.length > 0).map((def) => (
            <ThemedView key={def.key} type="backgroundElement" style={styles.row}>
              ...
              <Switch
                ...
                trackColor={{ false: colors.backgroundSelected, true: colors.highlight }}
      ...
      <View style={styles.actions}>
        ...
        <TouchableOpacity style={styles.skip} onPress={() => router.push('/(onboarding)/complete')}>
          <ThemedText type="small" themeColor="textSecondary">
            {t('onboarding.skip')}
          </ThemedText>
        </TouchableOpacity>
...
const styles = StyleSheet.create({
  ...
  row: { flexDirection: 'row', alignItems: 'center', padding: Spacing.three, borderRadius: 12, gap: Spacing.three },
  text: { flex: 1, gap: 2 },
  actions: { gap: Spacing.two, marginTop: Spacing.three },
  buttonInner: { padding: Spacing.three, borderRadius: 12, alignItems: 'center' },
